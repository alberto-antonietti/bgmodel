...
{ 
	...
	const double dop_GABAA_3 = 1 + node.P_.beta_I_GABAA_3*node.P_.tata_dop;
	...
	const double I_GABAA_3 = - y[S::G_GABAA_3] * ( V - node.P_.GABAA_3_E_rev )*dop_GABAA_3;

	// Dopamine modulation neuron
	const double k   = node.P_.k*(   1 - node.P_.beta_k*node.P_.tata_dop);
	const double E_L = node.P_.E_L*( 1 - node.P_.beta_E_L*node.P_.tata_dop);
	const double V_b = node.P_.V_b*( 1 - node.P_.beta_V_b*node.P_.tata_dop);
...
{
	def<double>(dd, "V_clamp",      V_clamp);
	def<double>(dd, "V_clamp_at",   V_clamp_at);

	def<double>(dd,names::V_m,          V_m);
	def<double>(dd, "E_L",      E_L);
	def<double>(dd, "V_th",      V_th);
	def<double>(dd,names::C_m,          C_m);
	def<double>(dd, "k",        k);
	def<double>(dd, "c",        c);
	def<double>(dd, "kc_1",       kc_1);
	def<double>(dd, "kc_2",       kc_2);
	def<double>(dd, "u_kc",       u_kc);
	def<double>(dd, "V_peak",   V_peak);
	def<double>(dd,names::I_e,          I_e);

	def<double>(dd, "u",      	 u);
	def<double>(dd, "V_b",       V_b);
	def<double>(dd, "a",         a);
	def<double>(dd, "b_1",       b_1);
	def<double>(dd, "b_2",       b_2);
	def<double>(dd, "p_1",       p_1);
	def<double>(dd, "p_2",       p_2);
	def<double>(dd, "d",         d);
	def<double>(dd, "u_const",   u_const);
	def<double>(dd, "u_max",     u_max);
	def<double>(dd, "c_max",		 c_max);

	def<double>(dd, "AMPA_1_E_rev",         AMPA_1_E_rev);
	def<double>(dd, "AMPA_1_Tau_decay",     AMPA_1_Tau_decay);

	def<double>(dd, "NMDA_1_E_rev",         NMDA_1_E_rev);
	def<double>(dd, "NMDA_1_Tau_decay",     NMDA_1_Tau_decay);
	def<double>(dd, "NMDA_1_Vact",          NMDA_1_Vact);
	def<double>(dd, "NMDA_1_Sact",       		NMDA_1_Sact);

	def<double>(dd, "GABAA_1_E_rev",     	GABAA_1_E_rev);
	def<double>(dd, "GABAA_1_Tau_decay", 	GABAA_1_Tau_decay);

	def<double>(dd, "GABAA_2_E_rev",     	GABAA_2_E_rev);
	def<double>(dd, "GABAA_2_Tau_decay", 	GABAA_2_Tau_decay);

	def<double>(dd, "GABAA_3_E_rev",     	GABAA_3_E_rev);
	def<double>(dd, "GABAA_3_Tau_decay", 	GABAA_3_Tau_decay);

	def<double>(dd, "tata_dop",		tata_dop);       //!< Proportion of open dopamine receptors. Zero as init since dopamine modulation is one then

	def<double>(dd, "beta_d",		beta_d);     //!< Dopamine effect on d
	def<double>(dd, "beta_k",		beta_k);     //!< Dopamine effect on k
	def<double>(dd, "beta_V_b",		beta_V_b);        //!< Dopamine effect on V_b
	def<double>(dd, "beta_E_L",		beta_E_L);        //!< Dopamine effect on E_L

	def<double>(dd, "beta_I_AMPA_1",		beta_I_AMPA_1);     //!< Dopamine effect on NMDA_1 current
	def<double>(dd, "beta_I_NMDA_1",		beta_I_NMDA_1);     //!< Dopamine effect on NMDA_1 current
	def<double>(dd, "beta_I_GABAA_1",	beta_I_GABAA_1);     //!< Dopamine effect GABAA 1 current
	def<double>(dd, "beta_I_GABAA_2",	beta_I_GABAA_2);     //!< Dopamine effect GABAA 2 current
	def<double>(dd, "beta_I_GABAA_3",	beta_I_GABAA_3);     //!< Dopamine effect GABAA 3 current

}

void mynest::izhik_cond_exp::Parameters_::set(const DictionaryDatum& dd)
{
	updateValue<double>(dd, "V_clamp",      V_clamp);
	updateValue<double>(dd, "V_clamp_at",   V_clamp_at);

	// allow setting the membrane potential
	updateValue<double>(dd,names::V_m,          V_m);
	updateValue<double>(dd, "E_L",      E_L);
	updateValue<double>(dd, "V_th",      V_th);
	updateValue<double>(dd,names::C_m,          C_m);
	updateValue<double>(dd, "c",        c);
	updateValue<double>(dd, "k",        k);
	updateValue<double>(dd, "kc_1",       kc_1 );
	updateValue<double>(dd, "kc_2",       kc_2 );
	updateValue<double>(dd, "u_kc",       u_kc );
	updateValue<double>(dd, "V_peak",   V_peak);
	updateValue<double>(dd,names::I_e,          I_e);

	updateValue<double>(dd, "u",         u);
	updateValue<double>(dd, "V_b",       V_b);
	updateValue<double>(dd, "a",         a);
	updateValue<double>(dd, "b_1",       b_1);
	updateValue<double>(dd, "b_2",       b_2);
	updateValue<double>(dd, "p_1",       p_1);
	updateValue<double>(dd, "p_2",       p_2);
	updateValue<double>(dd, "d",         d);
	updateValue<double>(dd, "u_const",   u_const);
	updateValue<double>(dd, "u_max",     u_max);
	updateValue<double>(dd, "c_max",		 c_max);

	updateValue<double>(dd, "AMPA_1_E_rev",        AMPA_1_E_rev);
	updateValue<double>(dd, "AMPA_1_Tau_decay",    AMPA_1_Tau_decay);

	updateValue<double>(dd, "NMDA_1_E_rev",        NMDA_1_E_rev);
	updateValue<double>(dd, "NMDA_1_Tau_decay",    NMDA_1_Tau_decay);
	updateValue<double>(dd, "NMDA_1_Vact",         NMDA_1_Vact);
	updateValue<double>(dd, "NMDA_1_Sact",         NMDA_1_Sact);

	updateValue<double>(dd, "GABAA_1_E_rev",     GABAA_1_E_rev);
	updateValue<double>(dd, "GABAA_1_Tau_decay", GABAA_1_Tau_decay);

	updateValue<double>(dd, "GABAA_2_E_rev",     GABAA_2_E_rev);
	updateValue<double>(dd, "GABAA_2_Tau_decay", GABAA_2_Tau_decay);

	updateValue<double>(dd, "GABAA_3_E_rev",     GABAA_3_E_rev);
	updateValue<double>(dd, "GABAA_3_Tau_decay", GABAA_3_Tau_decay);
	...
	updateValue<double>(dd, "beta_d",		beta_d);     //!< Dopamine effect on d
	updateValue<double>(dd, "beta_k",		beta_k);     //!< Dopamine effect on k
	updateValue<double>(dd, "beta_V_b",		beta_V_b);        //!< Dopamine effect on V_b
	updateValue<double>(dd, "beta_E_L",		beta_E_L);        //!< Dopamine effect on E_L

	updateValue<double>(dd, "beta_I_AMPA_1",		beta_I_AMPA_1);     //!< Dopamine effect on NMDA_1 current
	updateValue<double>(dd, "beta_I_NMDA_1",		beta_I_NMDA_1);     //!< Dopamine effect on NMDA_1 current
	updateValue<double>(dd, "beta_I_GABAA_1",	beta_I_GABAA_1);     //!< Dopamine effect GABAA 1 current
	updateValue<double>(dd, "beta_I_GABAA_2",	beta_I_GABAA_2);     //!< Dopamine effect GABAA 2 current
	updateValue<double>(dd, "beta_I_GABAA_3",	beta_I_GABAA_3);     //!< Dopamine effect GABAA 3 current
...
void mynest::izhik_cond_exp::update(Time const & origin, const long from, const long to)
{

	assert(to >= 0 && (delay) from < nest::kernel().connection_manager.get_min_delay());
	assert(from < to);
...
void mynest::izhik_cond_exp::handle(SpikeEvent & e)
{
	...
	if (e.get_rport() == AMPA_1 - MIN_SPIKE_RECEPTOR)
		B_.spikes_AMPA_1_.add_value(e.get_rel_delivery_steps(nest::kernel().simulation_manager.get_slice_origin()),
	...
	// If GABAA_3
		...
		B_.spikes_GABAA_3_.add_value(e.get_rel_delivery_steps(nest::kernel().simulation_manager.get_slice_origin()),
				e.get_weight() * e.get_multiplicity() );
